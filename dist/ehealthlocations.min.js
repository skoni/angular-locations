...
}}]),angular.module("eHealth.locations.services").provider("locations",function(){var a;this.setCountryCode=function(b){a=b},this.$get=["locationsFactory",function(b){if(a)return b(a);var c="a country code needs to be set via the locationsProvider. currently the country code is `"+JSON.stringify(a)+"`";throw new Error(c)}]}),angular.module("eHealth.locations.services").factory("locationsFactory",["$log","ml","gn","lr","sl","mg",function(a,b,c,d,e,f){var g={ml:b,gn:c,gin:c,lr:d,sl:e,mg:f};return function(b){if(b in g){var c=g[b],d=[];return d=c.map(function(a){var b={};return a.items.forEach(function(a){b[a.id]=a}),b}),c.decode=function(e,f){if("undefined"!=typeof e){var g=d[f];if(g){if(g[e])return g[e].name;a.error("we cannot find code "+e+" in locations level "+c[f].name)}else a.error(b+" locations have only "+d.length+" levels")}},c}var e="we have no location data for the country code `"+b+"`";throw new Error(e)}}]),angular.module("eHealth.locations.services").factory("selectedLocationFactory",["locations","$log",function(a,b){function c(d){function e(a){function b(a,c){0>c||a.isAll||(e(c),j[c].selectById(a.parentId),b(j[c].selected,c-1))}function c(a,b){var d=j[b];if(d){if(d.items=d.original.items.filter(function(b){return a.isAll||b.isAll||b.parentId===a.id}),d.selected)if(g)d.selected=d.items[0];else{var e;d.items.forEach(function(a){a.id===d.selected.id&&(e=!0)}),e||delete d.selected}c(d.items[0],b+1)}}var d=angular.copy(i[a]);if(g){var f={isAll:!0,name:"all"};d.items.unshift(f),d.selected=f}d.original=angular.copy(d),d.byId={},d.items.forEach(function(a){d.byId[a.id]=a}),d.selectById=function(a){var b=d.byId[a];if(!b)throw new Error("id "+a+" in not in level "+d.name);d.selected=d.byId[a]},d.update=function(){b(d.selected,a-1),c(d.selected,a+1)},j[a]=d}d=d||{};var f,g=d.hasAllItem,h=d.locationsData||a,i=angular.copy(h),j=[];return i.forEach(function(a,b){e(b)}),f={levels:j,select:function(a,b){j[a].selectById(b),j[a].update()},clone:function(){for(var a=c(d),b=0;b<a.levels.length;b++){var e=j[b].selected;e&&a.levels[b].selectById(e.id)}return a},compare:function(a){for(var b=j.length===a.levels.length,c=0;c<j.length;c++)angular.equals(j[c].selected,j[c].selected)||(b=!1),angular.equals(j[c].items,j[c].items)||(b=!1);return b},setAdminDivisions:function(a){if(a){var c=!0;[5,4,3,2,1].forEach(function(d){if(c){var e=d-1,g=a["adminDivision"+d];if(g)try{f.select(e,g),c=!1}catch(h){b.error(h)}}})}else b.error("`setAdminDivisions` was called on a `"+a+"` object");return f},getAdminDivisions:function(a){return a=a||{},j.forEach(function(b,c){if(b.selected){var d="adminDivision"+(c+1);a[d]=b.selected.id}}),angular.copy(a)}}}return c}]);